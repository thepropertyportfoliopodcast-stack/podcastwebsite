...
export default function TopicSuggestionForm({ episodeTitle = "" }) {
  ...
  return <section className="topic-suggestion relative isolate overflow-hidden rounded-[28px] border border-[#C347FF]/35 bg-[#120d1c] p-5 sm:p-8 lg:p-10">
    ...
    <div className="relative z-10 grid items-center gap-8 lg:grid-cols-[.78fr_1.22fr] lg:gap-12">
      <div><span className="inline-flex items-center gap-2 rounded-full border border-[#C347FF]/35 bg-white px-3 py-2 text-xs font-black tracking-[.15em] text-[#C347FF]"><FaStar aria-hidden="true"/>Your Idea, Our Next Conversation</span><h2 className="mt-5 text-[clamp(1.9rem,4vw,3.25rem)] font-black leading-[.98] tracking-[-.035em]">What Should We Talk About <span className="text-[#C347FF]">next?</span></h2><p className="mt-4 max-w-lg leading-7 text-white/65">Have a property question, market concern or investment idea? Send it directly to the team—it could inspire an upcoming episode.</p><div className="mt-6 flex items-center gap-3 text-sm font-bold text-[#C347FF]"><span className="flex h-10 w-10 items-center justify-center rounded-full text-[#C347FF] bg-white border-[#C347FF]/35 border"><FaLightbulb aria-hidden="true" /></span><span>Real listener ideas. Real expert conversations.</span></div></div>
