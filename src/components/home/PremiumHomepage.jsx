...
function AboutPodcast(){return <section id="home-about-podcast" className="home-section home-about"><div className="home-container grid items-center gap-10 lg:grid-cols-[.48fr_.52fr]"><div className="home-about-image relative aspect-[6/5] overflow-hidden rounded-[22px] border border-[#C347FF]/35 shadow-[0_20px_70px_rgba(99,23,140,.35)]"><Image src="/conversation.png" alt="Parag Dixit, Mudit Khandelwal and Julius Dabre" fill sizes="(max-width:1023px) calc(100vw - 40px), 560px" className="object-cover"/></div><div className="home-about-copy"><p className="home-eyebrow">About the podcast</p><h2 className="home-title">Property discussions built around real questions</h2><p className="home-copy mt-5">Our podcast brings real stories, fresh perspectives and direct conversations from voices that matter in Australian finance and property.</p><div className="home-about-features mt-8 grid gap-5 sm:grid-cols-3">{features.map(([Icon,title,text])=><div key={title}><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#A837F4]/15 text-[#DDBBFF]"><Icon size={21}/></span><h3 className="mt-3 font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-[#8F879B]">{text}</p></div>)}</div><Link href="/about" className="home-about-link mt-8 inline-flex items-center gap-2 border-b border-[#C347FF] pb-1 font-bold text-[#DDBBFF]">Meet the hosts <MdArrowOutward/></Link></div></div></section>}
function Episodes({episodes}){const source=Array.isArray(episodes)?episodes:[];const items=source.slice(0,5);const[active,setActive]=useState(0);const sectionRef=useRef(null);const[visible,setVisible]=useState(false);useEffect(()=>{const node=sectionRef.current;if(!node)return;const observer=new IntersectionObserver(([entry])=>setVisible(entry.isIntersecting),{rootMargin:"160px"});observer.observe(node);return()=>observer.disconnect()},[]);useEffect(()=>{if(items.length<2||!visible||document.visibilityState==="hidden")return;const timer=setInterval(()=>setActive((value)=>(value+1)%items.length),4200);return()=>clearInterval(timer)},[items.length,visible]);const move=(step)=>setActive((value)=>(value+step+items.length)%items.length);return <section ref={sectionRef} className="home-section home-episodes pt-0"><div className="home-container"><div className="flex items-end justify-between gap-5"><div><p className="home-eyebrow">Featured conversations</p><h2 className="home-title">Latest episodes</h2></div><Link href="/episode" className="mb-1 inline-flex min-h-11 shrink-0 items-center rounded-xl border border-[#C347FF]/45 px-5 text-sm font-bold hover:bg-[#A837F4]/10">View all episodes</Link></div>{items.length?<div className="featured-carousel mt-8" aria-roledescription="carousel"><div className="featured-carousel-stage">{items.map((episode,index)=>{let offset=index-active;if(offset>items.length/2)offset-=items.length;if(offset<-items.length/2)offset+=items.length;return <Link prefetch={false} key={episode.uuid||episode.id} href={contentPath("episode",episode)} className={`featured-carousel-card ${offset===0?"is-active":""}`} style={{"--offset":offset,"--distance":Math.abs(offset),opacity:offset===0?1:.58,zIndex:10-Math.abs(offset)}} aria-label={`Open ${episode.title}`}><div className="featured-carousel-image relative"><Image src={episode.thumbnail} alt={episode.title} fill quality={65} sizes="(max-width: 640px) 230px, (max-width: 1024px) 280px, 310px" className="object-cover"/><div className="featured-carousel-shade"/></div><div className="featured-carousel-copy"><p>{episode.topic||"Property insights"}</p><h3>{episode.title}</h3></div></Link>})}</div><div className="featured-carousel-nav"><button type="button" onClick={()=>move(-1)} aria-label="Previous featured episode">‹</button><div>{items.map((episode,index)=><button key={episode.uuid||episode.id} type="button" onClick={()=>setActive(index)} aria-label={`Show featured episode ${index+1}`} className={active===index?"is-active":""}/>)}</div><button type="button" onClick={()=>move(1)} aria-label="Next featured episode">›</button></div></div>:<p className="mt-8 text-center text-[#8F879B]">Featured episodes will appear here when selected in the dashboard.</p>}</div></section>}
