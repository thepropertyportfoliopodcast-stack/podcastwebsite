...
export async function downloadAnalyticsPdf({ analytics, audits = {}, pages = [], health = null }) {
  ...
  section("Traffic attribution by page", ["Source", "Medium", "Campaign", "Page", "Views", "Visitors", "Total engagement", "Average"], (analytics.sourcePages || []).map((row) => [row.source, row.medium, row.campaign, row.path, fixed(row.pageViews), fixed(row.visitors), seconds(row.totalEngagementSeconds), seconds(row.averageEngagementSeconds)]));
  section("Platform redirects", ["Platform", "Originating page", "Source", "Clicks", "Visitors", "Destination"], (analytics.platformConversions || []).map((row) => [row.platform, row.path, row.source, fixed(row.clicks), fixed(row.visitors), row.destination]));
  ...
  section("Self-hosted Lighthouse audits", ["Page", "Mode", "Performance", "Accessibility", "Best practices", "SEO", "LCP", "INP", "CLS", "TTFB"], auditRows.map(({ page, mode, result }) => [page.path, mode === "mobile" ? "Mobile" : "Desktop", text(result.scores?.performance), text(result.scores?.accessibility), text(result.scores?.["best-practices"]), text(result.scores?.seo), text(result.metrics?.lcp), text(result.metrics?.inp), text(result.metrics?.cls), text(result.metrics?.ttfb)]));
  section("Website health", ["Page", "Status", "HTTP", "Response time", "Checked", "Error"], (health?.pages || []).map((row) => [row.path, row.online ? "Online" : "Failing", text(row.status), `${fixed(row.responseTime)} ms`, row.checkedAt ? new Date(row.checkedAt).toLocaleString("en-AU") : "-", text(row.error)]));
