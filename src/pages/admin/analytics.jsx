...
import { MdAdsClick, MdDateRange, MdDevices, MdDownload, MdErrorOutline, MdOutlineRemoveRedEye, MdPeopleAlt, MdRefresh, MdSpeed, MdTimer } from "react-icons/md";
...
export default function AnalyticsPage() {
  ...
  const [pages, setPages] = useState([]); const [strategy, setStrategy] = useState("mobile"); const [audits, setAudits] = useState({}); const [auditLoading, setAuditLoading] = useState(false);
  ...
  useEffect(() => { api.analyticsPageSpeedPages().then((response) => setPages(response?.data?.data?.pages || [])).catch((err) => setError(err?.response?.data?.message || err.message)); loadHealth(); }, [api, loadHealth]);
  ...
  const auditVisible = useCallback(async (force = false) => {
    ...
    for (const page of pending) {
      ...
      try { const response = await api.analyticsPageSpeed({ url: page.url, strategy }); setAudits((current) => ({ ...current, [key]: response?.data?.data?.pageSpeed })); }
      catch (err) { setAudits((current) => ({ ...current, [key]: { error: err?.response?.data?.message || err.message } })); }
    }
    setAuditLoading(false);
  }, [api, audits, speedRows, strategy]);
  useEffect(() => { auditVisible(); }, [speedPage, strategy, pages.length]); // PageSpeed only; traffic analytics remains untouched.
