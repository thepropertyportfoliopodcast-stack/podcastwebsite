...
export default function AnalyticsPage() {
  ...
  return <AdminLayout><div className="admin-analytics-page mx-auto max-w-[1480px] space-y-4 text-slate-900">
    <header className="admin-page-header flex flex-wrap items-end justify-between gap-3 border-b border-violet-200 pb-4"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-700">Website intelligence</p><h1 className="mt-1 text-2xl font-black text-slate-950 md:text-3xl">Analytics & health centre</h1><p className="mt-1 text-sm text-slate-600">First-party traffic, real-user errors, uptime and self-hosted Lighthouse audits.</p>{analytics?.range && <p className="mt-2 inline-flex items-center gap-2 rounded-full bg-violet-100 px-3 py-1 text-xs font-bold text-violet-900"><MdDateRange />{analytics.range.startDate} to {analytics.range.endDate}</p>}</div><div className="admin-page-actions flex flex-wrap gap-2"><select value={preset} onChange={(event) => choosePreset(event.target.value)} className="rounded-lg border border-violet-300 bg-white px-3 py-2 text-sm text-slate-900"><option value="today">Today</option><option value="yesterday">Yesterday</option><option value="lastSevenDays">Last 7 days</option><option value="thisMonth">This month</option><option value="custom">Custom dates</option></select><button disabled={!analytics || exporting || loading} onClick={exportPdf} className="inline-flex items-center gap-2 rounded-lg border border-violet-400 bg-white px-4 py-2 text-sm font-bold text-violet-800 disabled:opacity-50"><MdDownload />{exporting ? "Preparing PDF…" : "Download PDF"}</button><button onClick={loadAnalytics} className="inline-flex items-center gap-2 rounded-lg bg-violet-700 px-4 py-2 text-sm font-bold text-white"><MdRefresh />Refresh traffic</button></div></header>
