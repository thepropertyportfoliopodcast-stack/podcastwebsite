        ...
        <div className="relative mx-auto max-w-[1310px] space-y-12 px-4">
          ...
          <section className="grid items-stretch gap-7 lg:grid-cols-[minmax(320px,.78fr)_minmax(0,2fr)]">
            <div className="flex min-h-0 flex-col">
              ...
              <div className="grid flex-1 gap-3">{related.length ? related.map((episode) => <Link key={episode.uuid} href={contentPath("episode", episode)} className="group grid min-h-[112px] grid-cols-[112px_minmax(0,1fr)] overflow-hidden rounded-2xl border border-white/15 bg-[#111] transition hover:-translate-y-0.5 hover:border-[#9747FF] sm:grid-cols-[132px_minmax(0,1fr)]"><div className="relative h-full min-h-[112px] w-full overflow-hidden"><Image src={episode.thumbnail} alt={`${episode.title} episode artwork`} fill sizes="(max-width: 640px) 112px, 132px" className="object-cover transition duration-300 group-hover:scale-105" /></div><div className="flex min-w-0 flex-col justify-center p-3 sm:p-4"><h3 className="line-clamp-3 text-sm font-bold leading-snug">{episode.title}</h3><div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[10px] font-semibold text-white/55"><span className="flex items-center gap-1"><FaClock className="shrink-0 text-[#c99cff]" aria-hidden="true" />{episodeDuration(episode.durationInSec, episode.duration)}</span>{episode.topic && <span className="flex min-w-0 items-center gap-1"><FaTag className="shrink-0 text-[#c99cff]" aria-hidden="true" /><span className="truncate">{episode.topic}</span></span>}</div></div></Link>) : <div className="min-h-[300px] rounded-2xl border border-white/15 bg-[#111] p-8 text-white/60">Related episodes will appear here.</div>}</div>
            </div>
            <div className="flex min-h-0 flex-col">
