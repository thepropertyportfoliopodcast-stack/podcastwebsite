export default function EpisodeRelationsFields({ formData, episodes = [], currentUuid, onChange }) {
  ...
  const toggleRelated = (uuid) => {
    const next = selected.includes(uuid) ? selected.filter((item) => item !== uuid) : [...selected, uuid].slice(0, 3);
  ...
  return <section className="space-y-5 rounded-xl border border-gray-800 bg-[#111] p-4 md:p-6">
    ...
    <div>
      <h4 className="font-semibold">Related episodes <span className="text-sm font-normal text-gray-400">({selected.length}/3)</span></h4>
      <p className="mb-3 mt-1 text-xs text-gray-400">Select up to three episodes. They will appear in this order on the episode page.</p>
      <div className="max-h-72 space-y-2 overflow-y-auto pr-2">
        {candidates.map((episode) => <label key={episode.uuid} className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-700 bg-[#1c1c1c] p-3">
          <input type="checkbox" checked={selected.includes(episode.uuid)} disabled={!selected.includes(episode.uuid) && selected.length >= 3} onChange={() => toggleRelated(episode.uuid)} className="h-4 w-4 accent-[#9747FF]" />
